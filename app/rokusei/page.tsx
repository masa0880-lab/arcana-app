...
export default function RokuseiPage() {
  const [phase, setPhase] = useState<Phase>('input');
  const [birth, setBirth] = useState<Partial<BirthDate>>({});
  const [result, setResult] = useState<RokuseiResponse | null>(null);
  ...
  async function handleSubmit() {
    ...
    setPhase('loading');
    try {
      const res = await fetch('/api/rokusei', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ birth }),
      });
      const data = (await res.json().catch(() => ({}))) as Partial<RokuseiResponse> & {
        error?: string;
      };
      if (
        !res.ok ||
        ...
        !data.polarity ||
        !data.currentCycleId ||
        typeof data.fortuneNumber !== 'number'
      ) {
        throw new Error(data.error ?? `HTTP ${res.status}`);
      }
      const payload: RokuseiResponse = {
        starId: data.starId,
        polarity: data.polarity,
        fortuneNumber: data.fortuneNumber,
      ...
      const entry: RokuseiHistoryEntry = {
        kind: 'rokusei',
        id: uuid(),
        ...
        fortuneNumber: payload.fortuneNumber,
        currentCycleId: payload.currentCycleId,
    ...
    return (
      <div className="space-y-8">
        <header className="space-y-2 text-center">
          <p className="text-xs uppercase tracking-[0.3em] text-arcana-accent">SIX STARS</p>
          <h1 className="font-serif text-3xl text-arcana-text">六星占術</h1>
          <p className="text-sm text-arcana-muted">
            生年月日から、あなたの星と今年の運命周期を読み解きます
          </p>
        </header>
        ...
        <p className="rounded-lg border border-white/5 bg-arcana-surface/30 px-4 py-3 text-[10px] text-arcana-muted">
          ※ 本サービスは細木数子氏/同名団体の登録商標「六星占術®」とは無関係の独立した鑑定です。鑑定文はClaudeによるオリジナル生成です。
        </p>
      </div>
    );
  }
  ...
  if (phase === 'result' && result && submittedBirth) {
    ...
    const cycle = getRokuseiCycle(result.currentCycleId);
    if (!star || !cycle) {
      return <p className="text-center text-arcana-muted">星データが見つかりません</p>;
    }
    return (
      <div className="space-y-8">
        <motion.section
        ...
        >
          ...
          <div className="text-center">
            ...
            <h2 className="font-serif text-2xl text-arcana-accent">
              {star.nameJa}（{result.polarity}）
            </h2>
        ...
        <motion.section
          ...
          className={`rounded-2xl border p-4 text-center ${
            cycle.isDaisakkai
              ? 'border-arcana-danger/40 bg-arcana-danger/5'
              : 'border-white/5 bg-arcana-surface/40'
          }`}
        >
          <p className="text-[10px] uppercase tracking-[0.2em] text-arcana-muted">
            {new Date().getFullYear()}年の運命周期
          </p>
          <p className="font-serif text-xl text-arcana-accent">{cycle.nameJa}</p>
          {cycle.isDaisakkai && (
            <p className="mt-1 text-[10px] text-arcana-danger">
              ※ 警戒期 — 内省と整理に向く時期
            </p>
          )}
        </motion.section>
        ...
        <p className="rounded-lg border border-white/5 bg-arcana-surface/30 px-4 py-3 text-[10px] text-arcana-muted">
          ※ 本サービスは細木数子氏/同名団体の登録商標「六星占術®」とは無関係の独立した鑑定です。
        </p>
      </div>
    );
  }
