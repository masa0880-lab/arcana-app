...
export default function AnimalPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const data = (await res.json().catch(() => ({}))) as Partial<AnimalResponse> & {
        error?: string;
      };
      if (!res.ok || !data.interpretation || !data.animalId || typeof data.characterNumber !== 'number') {
        throw new Error(data.error ?? `HTTP ${res.status}`);
      }
      const payload: AnimalResponse = {
    ...
    return (
      <div className="space-y-8">
        <header className="space-y-2 text-center">
          <p className="text-xs uppercase tracking-[0.3em] text-arcana-accent">ANIMAL</p>
          <h1 className="font-serif text-3xl text-arcana-text">動物占い</h1>
          <p className="text-sm text-arcana-muted">
            生年月日から、あなたの内なる動物キャラクターを読み解きます
          </p>
        </header>
        ...
        <p className="rounded-lg border border-white/5 bg-arcana-surface/30 px-4 py-3 text-[10px] text-arcana-muted">
          ※ 本サービスは個性心理學研究所の「動物占い®」とは無関係の独立した鑑定です。鑑定文はClaudeによるオリジナル生成です。
        </p>
      </div>
    );
  }
  ...
  if (phase === 'result' && result && submittedBirth) {
    ...
    if (!animal) {
      return <p className="text-center text-arcana-muted">動物データが見つかりません</p>;
    }
    return (
      <div className="space-y-8">
        <motion.section
        ...
        >
          ...
          <div className="text-center">
            ...
            <h2 className="font-serif text-2xl text-arcana-accent">{animal.nameJa}</h2>
        ...
        <p className="rounded-lg border border-white/5 bg-arcana-surface/30 px-4 py-3 text-[10px] text-arcana-muted">
          ※ 本サービスは個性心理學研究所の「動物占い®」とは無関係の独立した鑑定です。
        </p>
      </div>
    );
  }
